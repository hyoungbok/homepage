$(function(){
    ...
    /* 브랜트스토리 호버효과*/
	$('#main_goods>a>div').each(function(){
		$(this).hover(
			function(){
				$(this).find('img').stop().fadeIn().css({'display':'block','position':'absolute','z-index':'1'})
			},
			function(){
				$(this).find('img').stop().fadeOut().css({'display':'none','position':'absolute','z-index':'1'})
			}
		);
	})
	$('.box1').hover(
		function(){
			$('.box1_img>.g_right').stop().animate({'right':'0'},500);
			$('.box1_img>.g_left').stop().animate({'left':'0'},500);
		},
		function(){
			$('.box1_img>.g_right').stop().animate({'right':'-510px'},200);
			$('.box1_img>.g_left').stop().animate({'left':'-410px'},200);
		}
	);
	$('.box2').hover(
		function(){
			$('.box2_img>.g_right').stop().animate({'right':'0'},500);
			$('.box2_img>.g_left').stop().animate({'left':'0'},500);
		},
		function(){
			$('.box2_img>.g_right').stop().animate({'right':'-510px'},200);
			$('.box2_img>.g_left').stop().animate({'left':'-410px'},200);
		}
	);	
	$('.box3').hover(
		function(){
			$('.box3_img>.g_right').stop().animate({'right':'0'},500);
			$('.box3_img>.g_left').stop().animate({'left':'0'},500);
		},
		function(){
			$('.box3_img>.g_right').stop().animate({'right':'-510px'},200);
			$('.box3_img>.g_left').stop().animate({'left':'-410px'},200);
		}
	);	
	/* 롤링 버튼 배너 */
	/* 숫자버튼 눌러이미지 변화 */
